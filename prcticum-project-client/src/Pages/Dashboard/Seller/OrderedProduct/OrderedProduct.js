...
const OrderedProduct = () => {
  ...
  const {
    ...
    queryFn: async () => {
      const res = await fetch(
        `${process.env.REACT_APP_API_URL}/product-order?email=${user?.email}`,
        {
          headers: {
  ...
  useEffect(() => {
      ...
      fetch(`${process.env.REACT_APP_API_URL}/delivery-man`)
        .then((res) => res.json())
        .then((data) => {
  ...
  const handleAssignDeliveryMan = (deliveryManEmail, orderId) => {
    ...
    if (
      ...
      fetch(`${process.env.REACT_APP_API_URL}/assign-delivery-man/${orderId}`, {
        method: "PATCH",
        headers: {
