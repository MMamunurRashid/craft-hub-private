  ...
  try {
            ...
            const result = await productsCollection.updateOne(
              { _id: productId },
              {
                $inc: {
                  // saleQuantity: 1,
                  // availableQuantity: -1,
                  saleQuantity: parseInt(product.quantity),
          ...
          const result = await productsCollection.updateOne(
            { _id: productId },
            {
              $inc: {
                // saleQuantity: 1,
                // availableQuantity: -1,
                saleQuantity: parseInt(query.products.quantity),
                availableQuantity: -parseInt(query.products.quantity),
              },
            }
          );
    ...
    app.post("/orders-payment", async (req, res) => {
      ...
      const data = {
        ...
        success_url: `${process.env.BACKEND_URL}/payment/success?transactionId=${transactionId}`,
    ...
    app.post("/payment/success", async (req, res) => {
      ...
      if (!transactionId) {
        return res.redirect(`${process.env.FRONTEND_URL}/dashboard/my-order`);
      }
      ...
      if (result.modifiedCount > 0) {
        res.redirect(`${process.env.FRONTEND_URL}/dashboard/my-order`);
      }
    });
